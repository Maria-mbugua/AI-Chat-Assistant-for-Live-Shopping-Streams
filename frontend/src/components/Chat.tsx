...
export const Chat: React.FC = () => {
    ...
    const processImageScan = (imageData: string, userNotes?: string) => {
        setIsTyping(true);
        setTimeout(() => {
            let reply = "";
            ...
            if (notes.includes('material') || notes.includes('fabric')) {
                ...
                const replies = [
                    "I've scanned your screenshot! That's our **Cyberpunk Leather Jacket**. It's incredibly stylish and fits true to size! 🔥",
                    "Analyzing image... Spotted! That's the **Urban Street Hoodie**. We have it in all sizes from S to XL.",
                    "Recognized! Those are the **Premium Canvas Sneakers**. They're extremely popular right now. 🌍"
                ];
    ...
    const sendMessage = (e: React.FormEvent) => {
        ...
        if (ws.current && ws.current.readyState === WebSocket.OPEN) {
            ws.current.send(JSON.stringify({ text: currentInput, image: currentImage }));
            sentViaWS = true;
        }
